...
# -----------------------------------------------------------
# 3 RBAC roles to SP at scope 'Tenant Root Group'
...
# Entra roles id = portal > role > get template ID
# role id = templeate id
# ----------------------------------
...
resource "azuread_directory_role_assignment" "directory_reader_to_github_sp" {
  role_id             = "88d8e3e3-8f55-4a1e-953a-9b9898b8876b" //ID of Directory Readers role
...
resource "azurerm_role_assignment" "storage_blob_data_contributor_to_github_sp" {
  ...
  scope                = "/subscriptions/3c341c66-2169-45ff-9554-8fad1cba9202/resourceGroups/alz-cicd-remote-backend/providers/Microsoft.Storage/storageAccounts/alzcicdstorage"
  principal_id         = azuread_service_principal.github_sp.object_id //object id of github SP
